...
static void schedule(unsigned int cpu_id)
{
	...
	if (ready_queue_head != NULL){					/*if the ready queue is not empty*/
...
extern void idle(unsigned int cpu_id)
{
	...
	while( ready_queue_head == NULL && ready_queue_tail == NULL){	/*while the queue is empty*/
...
extern void wake_up(pcb_t *process)
{
	...
	if(ready_queue_tail == NULL)
		printf("    Tail NULL\n");
	if(ready_queue_head != NULL)
		printf("Head of queue is %s\n", ready_queue_head->name);
	if(ready_queue_tail != NULL)
		printf("Tail of queue is %s\n", ready_queue_head->name);

	pthread_mutex_lock(&rqueue_mutex);			/*lock the ready queue*/
	if (ready_queue_head==NULL && ready_queue_tail==NULL){	/*if the ready queue is empty*/
		...
		printf("%s is ready, inserting it on the tail\n", process->name);
		if(ready_queue_head == NULL){
