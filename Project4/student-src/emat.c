...
double compute_emat() {
   ...
   double EMAT;
   long int tlb_misses;
   long int total_accesses;
   total_accesses = count_writes + count_reads;
   tlb_misses = total_accesses - count_tlbhits;
   EMAT = 2*MEMORY_ACCESS_TIME*tlb_misses + MEMORY_ACCESS_TIME*count_tlbhits + 
		(2*MEMORY_ACCESS_TIME + DISK_ACCESS_TIME)*count_pagefaults;
